...
for kp_dir in keypoint_dirs:
    ...
    if frames:
        ...
        print(f"Skipping empty folder: {kp_dir}")

expected_videos = grid_size * grid_size
if len(frame_lists) < expected_videos:
    print(f"Only {len(frame_lists)} videos found, padding with empty placeholders.")
...
for i in range(min_frame_count):
    ...
    print(f"Saved collage frame: {collage_path}")

print("Creating final collage video...")
